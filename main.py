...
from .core.geo import LocationResolver
from .core.push_service import ActivePushService
...
class WeatherAssistantPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self._location_resolver = LocationResolver(
            self._qweather_client, self._weather_config
        )
        self._push_service = ActivePushService(
            context=self.context,
            weather_config=self._weather_config,
            qweather_client=self._qweather_client,
    ...
    async def weather_now(self, event: AstrMessageEvent, location: str = ""):
        ...
        """
        location_input = (
            location.strip() if location else self._location_resolver.default_location()
        )
    ...
    async def minutely_precip(self, event: AstrMessageEvent, location: str = ""):
        ...
        try:
            (
                display_name,
                _,
            ) = await self._location_resolver.resolve_display_name_for_lonlat(
                location_input
            )
